...
{
    QString ret = QString("cryptographicanomaly:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CGA unit input in lnReqAmount, we generate the URI with CGA as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(CryptographicanomalyUnits::format(CryptographicanomalyUnits::CGA, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
