...
uint256 hashGenesisBlock("0xffe6d5d389266a462f8260a3358ab05571c030bb5c0b5d9fd58f5961e2e50290");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cryptographicanomaly: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cryptographicanomaly Signed Message:\n";
...
{
    // Cryptographicanomaly: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	else if(block <= 155500 )					// Update 1.3.3.1
	{
		if(diff <= 3)
		{
			nSubsidy = 0.33333333 * COIN;
	...
	else if(block > 155500 )					// Update 1.3.4.1
	{	double rDiff = floor( pow(10.0,8) * diff ) / pow(10.0,8);
		if(rDiff <= 3)
		{nSubsidy = 0.33333333 * COIN;}
		else if(rDiff > 3)
		{
...
static const int64 nTargetTimespan = 80; // Cryptographicanomaly: 80 sec
static const int64 nTargetSpacing = 40; // Cryptographicanomaly: 40 sec
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cryptographicanomaly: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("cryptographicanomaly-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cryptographicanomaly: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Cryptographicanomaly: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd02602d8812b813b10526d49a92a2b0746dfa1234d2ee62faec57c54a9ef4eb0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1391272153;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 394322507;

        if (fTestNet)
        {
            block.nTime    = 1391272153;
            block.nNonce   = 388539783;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x91fc86966f4f0ea162a071796998477b43ac1c8dc980a181797bdfe418dc1127"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Cryptographicanomaly: increase each by adding 2 to cryptographicanomaly's value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (strstr(pfrom->cleanSubVer.c_str(), "Reddcoin") != NULL) {
            // disconnect Reddcoin clients
            printf("partner %s is not CGA client; disconnecting\n", pfrom->addr.ToString().c_str());
            pfrom->fDisconnect = true;
            return false;
        }
        if (!vRecv.empty())
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    printf("CryptographicanomalyMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryptographicanomalyMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryptographicanomalyMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CryptographicanomalyMiner(CWallet *pwallet)
{
    printf("CryptographicanomalyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cryptographicanomaly-miner");
    ...
    try { loop {
        ...
        printf("Running CryptographicanomalyMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CryptographicanomalyMiner terminated\n");
        throw;
    }
}

void GenerateCryptographicanomalys(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CryptographicanomalyMiner, pwallet));
}
