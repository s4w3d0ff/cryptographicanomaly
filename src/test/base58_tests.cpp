...
BOOST_AUTO_TEST_CASE(base58_keys_valid_parse)
{
    ...
    std::vector<unsigned char> result;
    CCryptographicanomalySecret secret;
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if(isPrivkey)
        {
            ...
            // Note: CCryptographicanomalySecret::SetString tests isValid, whereas CCryptographicanomalyAddress does not!
...
BOOST_AUTO_TEST_CASE(base58_keys_valid_gen)
{
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        else
        {
            ...
            CCryptographicanomalyAddress addrOut;
            BOOST_CHECK_MESSAGE(boost::apply_visitor(CCryptographicanomalyAddressVisitor(&addrOut), dest), "encode dest: " + strTest);
            BOOST_CHECK_MESSAGE(addrOut.ToString() == exp_base58string, "mismatch: " + strTest);
        }
    }

    // Visiting a CNoDestination must fail
    CCryptographicanomalyAddress dummyAddr;
    CTxDestination nodest = CNoDestination();
    BOOST_CHECK(!boost::apply_visitor(CCryptographicanomalyAddressVisitor(&dummyAddr), nodest));
